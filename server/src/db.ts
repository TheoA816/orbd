...
const getUserByToken = (token: string): Promise<null | User> => {
  const res = pool
    .query(`SELECT * FROM Users WHERE refreshToken ~* ('^' || $1 || '$')`, [
      token,
    ])
    .then((res) => {
      // should only return one result if exists
...
const registerUser = async (
  ...
  await pool
    .query(`INSERT INTO Stats (id, best_time, plays) VALUES ($1, $2, $3)`, [
      id,
      null,
      0,
    ])
...
const updateToken = async (refreshToken: string, email: string) => {
  await pool
    .query(`UPDATE Users SET refreshToken = $1 WHERE email = $2`, [
      refreshToken,
      email,
    ])

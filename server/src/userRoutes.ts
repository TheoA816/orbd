...
router.post("/signout", (req, res, next) => {
  // res.clearCookie('jwt', { httpOnly: true, secure: true, sameSite: 'none', path: '/', domain: 'orbd-be.onrender.com', maxAge: 0 })
  res.clearCookie("jwt", {
    httpOnly: true,
    secure: true,
    sameSite: "none",
...
router.get("/stats", async (req, res, next) => {
  ...
  if (user === null) {
    res
      .status(403)
      .json({ error: "more or less than one user found - forbidden" });
    return;
  }
